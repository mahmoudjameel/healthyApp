...
class DiseaseProductsScreen extends Component{
  state={
    ...
    constructor (props){
      ...
      render(){
        ...
        const diseaseProducts = this.props.availableProducts.availableProducts
        
        if(diseaseProducts.length ===0){
          return(
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Text>No products found may be we will add some!</Text>
          </View> 
          )
        }
        ...
        const renderProductItem = itemData =>{
          const isProduct =  itemData.item.productdiseases_set.some(prod=>prod.disease_category.id === this.state.diseaseId)
          if(!isProduct){
...
DiseaseProductsScreen.navigationOptions = navigationData => {
  return {
...
export default connect(mapStateToProps,mapDispatchToProps)(DiseaseProductsScreen);